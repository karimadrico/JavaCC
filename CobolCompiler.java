/* CobolCompiler.java */
/* Generated By:JavaCC: Do not edit this line. CobolCompiler.java */
public class CobolCompiler implements CobolCompilerConstants {
    // Lista para almacenar el código intermedio generado
    private java.util.List<String> codigoIntermedio = new java.util.ArrayList<>();
  public static void main(String[] args) throws Exception {
    CobolCompiler parser = new CobolCompiler(System.in);
    parser.Program();
    System.out.println("Programa analizado correctamente.\n\nCódigo intermedio generado:");
    for (String linea : parser.codigoIntermedio) {
      System.out.println(linea);
    }
  }

/* ====== GRAMÁTICA ====== */
  final public void Program() throws ParseException {Token progName;
    jj_consume_token(PROGRAM);
    progName = jj_consume_token(ID);
    jj_consume_token(DOT);
    jj_consume_token(BEGIN);
    stmts();
    jj_consume_token(END);
    jj_consume_token(DOT);
}

  final public void stmts() throws ParseException {
    label_1:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
      case LEE:
      case MUESTRA:
      case SUMA:
      case RESTA:
      case MULTIPLICA:
      case DIVIDE:
      case MUEVE:
      case CALCULA:
      case SI:
      case EJECUTA:
      case ID:{
        ;
        break;
        }
      default:
        jj_la1[0] = jj_gen;
        break label_1;
      }
      stmt();
    }
}

  final public void stmt() throws ParseException {
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case EJECUTA:
    case ID:{
      loop();
      jj_consume_token(DOT);
      break;
      }
    case SI:{
      cond();
      jj_consume_token(DOT);
      break;
      }
    case SUMA:
    case RESTA:
    case MULTIPLICA:
    case DIVIDE:
    case MUEVE:
    case CALCULA:{
      assig();
      jj_consume_token(DOT);
      break;
      }
    case LEE:
    case MUESTRA:{
      io();
      jj_consume_token(DOT);
      break;
      }
    default:
      jj_la1[1] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
}

// Entrada y salida
  final public void io() throws ParseException {
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case LEE:{
      jj_consume_token(LEE);
      jj_consume_token(ID);
      break;
      }
    case MUESTRA:{
      jj_consume_token(MUESTRA);
      literal();
      label_2:
      while (true) {
        switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
        case COMA:{
          ;
          break;
          }
        default:
          jj_la1[2] = jj_gen;
          break label_2;
        }
        jj_consume_token(COMA);
        literal();
      }
      break;
      }
    default:
      jj_la1[3] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
}

// Un literal puede ser ID, NUM o cadena
  final public void literal() throws ParseException {
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case ID:{
      jj_consume_token(ID);
      break;
      }
    case NUM:{
      jj_consume_token(NUM);
      break;
      }
    default:
      jj_la1[4] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
}

// Asignaciones: SUMA y RESTA
  final public void assig() throws ParseException {
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case SUMA:{
      Token t = jj_consume_token(SUMA);
      Token v1 = valor();
      java.util.List<Token> otros = new java.util.ArrayList<>();
      label_3:
      while (true) {
        switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
        case PARENOPEN:
        case ID:
        case NUM:{
          ;
          break;
          }
        default:
          jj_la1[5] = jj_gen;
          break label_3;
        }
        otros.add(valor());
      }
      jj_consume_token(A);
      Token res = jj_consume_token(ID);
      StringBuilder sb = new StringBuilder();
      sb.append(res.image).append(" = ").append(v1.image);
      for (Token ot : otros) sb.append(" + ").append(ot.image);
      codigoIntermedio.add(sb.toString());
      break;
      }
    case RESTA:{
      Token t = jj_consume_token(RESTA);
      Token v1 = valor();
      java.util.List<Token> otros = new java.util.ArrayList<>();
      label_4:
      while (true) {
        switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
        case PARENOPEN:
        case ID:
        case NUM:{
          ;
          break;
          }
        default:
          jj_la1[6] = jj_gen;
          break label_4;
        }
        otros.add(valor());
      }
      jj_consume_token(DE);
      Token res = jj_consume_token(ID);
      StringBuilder sb = new StringBuilder();
      sb.append(res.image).append(" = ").append(v1.image);
      for (Token ot : otros) sb.append(" - ").append(ot.image);
      codigoIntermedio.add(sb.toString());
      break;
      }
    case MULTIPLICA:{
      Token t = jj_consume_token(MULTIPLICA);
      Token v1 = valor();
      jj_consume_token(POR);
      Token res = jj_consume_token(ID);
      codigoIntermedio.add(res.image + " = " + v1.image + " * ...");
      break;
      }
    case DIVIDE:{
      Token t = jj_consume_token(DIVIDE);
      Token v1 = valor();
      jj_consume_token(ENTRE);
      Token res = jj_consume_token(ID);
      codigoIntermedio.add(res.image + " = " + v1.image + " / ...");
      break;
      }
    case MUEVE:{
      Token t = jj_consume_token(MUEVE);
      Token v1 = valor();
      jj_consume_token(A);
      Token res = jj_consume_token(ID);
      codigoIntermedio.add(res.image + " = " + v1.image);
      break;
      }
    case CALCULA:{
      Token t = jj_consume_token(CALCULA);
      Token res = jj_consume_token(ID);
      switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
      case IGUAL_OP:{
        jj_consume_token(IGUAL_OP);
        Token v = valor();
        jj_consume_token(FIN_CALCULA);
        codigoIntermedio.add(res.image + " = " + v.image);
        break;
        }
      case COMO:{
        jj_consume_token(COMO);
        Token v = valor();
        codigoIntermedio.add(res.image + " = " + v.image);
        break;
        }
      default:
        jj_la1[7] = jj_gen;
        jj_consume_token(-1);
        throw new ParseException();
      }
      break;
      }
    default:
      jj_la1[8] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
}

// Un valor puede ser ID, NUM o una expresión entre paréntesis
  final public void valor() throws ParseException {
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case ID:{
      return jj_consume_token(ID);
      }
    case NUM:{
      return jj_consume_token(NUM);
      }
    case PARENOPEN:{
      jj_consume_token(PARENOPEN);
      Token t = valor();
      jj_consume_token(PARENCLOSE);
      return t;
      }
    default:
      jj_la1[9] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
}

  final public void loop() throws ParseException {
    // Ejemplo: agregar comentario de bucle al código intermedio
    codigoIntermedio.add("# INICIO BUCLE");
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case EJECUTA:{
      jj_consume_token(EJECUTA);
      stmts();
      jj_consume_token(HASTA_QUE);
      cond();
      jj_consume_token(FIN_EJECUTA);
      codigoIntermedio.add("# FIN BUCLE WHILE");
      break;
      }
    case ID:{
      jj_consume_token(ID);
      jj_consume_token(IGUAL_OP);
      jj_consume_token(NUM);
      switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
      case A:{
        jj_consume_token(A);
        jj_consume_token(NUM);
        switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
        case POR:{
          jj_consume_token(POR);
          jj_consume_token(NUM);
          break;
          }
        default:
          jj_la1[10] = jj_gen;
          ;
        }
        break;
        }
      default:
        jj_la1[11] = jj_gen;
        ;
      }
      jj_consume_token(VECES);
      stmts();
      jj_consume_token(FIN_EJECUTA);
      codigoIntermedio.add("# FIN BUCLE FOR");
      break;
      }
    default:
      jj_la1[12] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
}

  final public void cond() throws ParseException {
    codigoIntermedio.add("# INICIO IF");
    jj_consume_token(SI);
    expr();
    jj_consume_token(ENTONCES);
    stmts();
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case SINO:{
      jj_consume_token(SINO);
      stmts();
      codigoIntermedio.add("# ELSE");
      break;
      }
    default:
      jj_la1[13] = jj_gen;
      ;
    }
    jj_consume_token(FIN_SI);
    codigoIntermedio.add("# FIN IF");
}

// Expresión booleana simple para condicionales y bucles
  final public void expr() throws ParseException {
    valor();
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case MAYOR:{
      jj_consume_token(MAYOR);
      break;
      }
    case MENOR:{
      jj_consume_token(MENOR);
      break;
      }
    case IGUAL:{
      jj_consume_token(IGUAL);
      break;
      }
    default:
      jj_la1[14] = jj_gen;
      jj_consume_token(-1);
      throw new ParseException();
    }
    valor();
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case NO:{
      jj_consume_token(NO);
      break;
      }
    default:
      jj_la1[15] = jj_gen;
      ;
    }
}

  /** Generated Token Manager. */
  public CobolCompilerTokenManager token_source;
  SimpleCharStream jj_input_stream;
  /** Current token. */
  public Token token;
  /** Next token. */
  public Token jj_nt;
  private int jj_ntk;
  private int jj_gen;
  final private int[] jj_la1 = new int[16];
  static private int[] jj_la1_0;
  static private int[] jj_la1_1;
  static {
	   jj_la1_init_0();
	   jj_la1_init_1();
	}
	private static void jj_la1_init_0() {
	   jj_la1_0 = new int[] {0x2203fc00,0x2203fc00,0x0,0xc00,0x0,0x0,0x0,0x40000,0x3f000,0x0,0x400000,0x200000,0x20000000,0x8000000,0x0,0x0,};
	}
	private static void jj_la1_init_1() {
	   jj_la1_1 = new int[] {0x10000,0x10000,0x100,0x0,0x30000,0x32000,0x32000,0x8000,0x0,0x32000,0x0,0x0,0x10000,0x0,0x70,0x4,};
	}

  /** Constructor with InputStream. */
  public CobolCompiler(java.io.InputStream stream) {
	  this(stream, null);
  }
  /** Constructor with InputStream and supplied encoding */
  public CobolCompiler(java.io.InputStream stream, String encoding) {
	 try { jj_input_stream = new SimpleCharStream(stream, encoding, 1, 1); } catch(java.io.UnsupportedEncodingException e) { throw new RuntimeException(e); }
	 token_source = new CobolCompilerTokenManager(jj_input_stream);
	 token = new Token();
	 jj_ntk = -1;
	 jj_gen = 0;
	 for (int i = 0; i < 16; i++) jj_la1[i] = -1;
  }

  /** Reinitialise. */
  public void ReInit(java.io.InputStream stream) {
	  ReInit(stream, null);
  }
  /** Reinitialise. */
  public void ReInit(java.io.InputStream stream, String encoding) {
	 try { jj_input_stream.ReInit(stream, encoding, 1, 1); } catch(java.io.UnsupportedEncodingException e) { throw new RuntimeException(e); }
	 token_source.ReInit(jj_input_stream);
	 token = new Token();
	 jj_ntk = -1;
	 jj_gen = 0;
	 for (int i = 0; i < 16; i++) jj_la1[i] = -1;
  }

  /** Constructor. */
  public CobolCompiler(java.io.Reader stream) {
	 jj_input_stream = new SimpleCharStream(stream, 1, 1);
	 token_source = new CobolCompilerTokenManager(jj_input_stream);
	 token = new Token();
	 jj_ntk = -1;
	 jj_gen = 0;
	 for (int i = 0; i < 16; i++) jj_la1[i] = -1;
  }

  /** Reinitialise. */
  public void ReInit(java.io.Reader stream) {
	if (jj_input_stream == null) {
	   jj_input_stream = new SimpleCharStream(stream, 1, 1);
	} else {
	   jj_input_stream.ReInit(stream, 1, 1);
	}
	if (token_source == null) {
 token_source = new CobolCompilerTokenManager(jj_input_stream);
	}

	 token_source.ReInit(jj_input_stream);
	 token = new Token();
	 jj_ntk = -1;
	 jj_gen = 0;
	 for (int i = 0; i < 16; i++) jj_la1[i] = -1;
  }

  /** Constructor with generated Token Manager. */
  public CobolCompiler(CobolCompilerTokenManager tm) {
	 token_source = tm;
	 token = new Token();
	 jj_ntk = -1;
	 jj_gen = 0;
	 for (int i = 0; i < 16; i++) jj_la1[i] = -1;
  }

  /** Reinitialise. */
  public void ReInit(CobolCompilerTokenManager tm) {
	 token_source = tm;
	 token = new Token();
	 jj_ntk = -1;
	 jj_gen = 0;
	 for (int i = 0; i < 16; i++) jj_la1[i] = -1;
  }

  private Token jj_consume_token(int kind) throws ParseException {
	 Token oldToken;
	 if ((oldToken = token).next != null) token = token.next;
	 else token = token.next = token_source.getNextToken();
	 jj_ntk = -1;
	 if (token.kind == kind) {
	   jj_gen++;
	   return token;
	 }
	 token = oldToken;
	 jj_kind = kind;
	 throw generateParseException();
  }


/** Get the next Token. */
  final public Token getNextToken() {
	 if (token.next != null) token = token.next;
	 else token = token.next = token_source.getNextToken();
	 jj_ntk = -1;
	 jj_gen++;
	 return token;
  }

/** Get the specific Token. */
  final public Token getToken(int index) {
	 Token t = token;
	 for (int i = 0; i < index; i++) {
	   if (t.next != null) t = t.next;
	   else t = t.next = token_source.getNextToken();
	 }
	 return t;
  }

  private int jj_ntk_f() {
	 if ((jj_nt=token.next) == null)
	   return (jj_ntk = (token.next=token_source.getNextToken()).kind);
	 else
	   return (jj_ntk = jj_nt.kind);
  }

  private java.util.List<int[]> jj_expentries = new java.util.ArrayList<int[]>();
  private int[] jj_expentry;
  private int jj_kind = -1;

  /** Generate ParseException. */
  public ParseException generateParseException() {
	 jj_expentries.clear();
	 boolean[] la1tokens = new boolean[50];
	 if (jj_kind >= 0) {
	   la1tokens[jj_kind] = true;
	   jj_kind = -1;
	 }
	 for (int i = 0; i < 16; i++) {
	   if (jj_la1[i] == jj_gen) {
		 for (int j = 0; j < 32; j++) {
		   if ((jj_la1_0[i] & (1<<j)) != 0) {
			 la1tokens[j] = true;
		   }
		   if ((jj_la1_1[i] & (1<<j)) != 0) {
			 la1tokens[32+j] = true;
		   }
		 }
	   }
	 }
	 for (int i = 0; i < 50; i++) {
	   if (la1tokens[i]) {
		 jj_expentry = new int[1];
		 jj_expentry[0] = i;
		 jj_expentries.add(jj_expentry);
	   }
	 }
	 int[][] exptokseq = new int[jj_expentries.size()][];
	 for (int i = 0; i < jj_expentries.size(); i++) {
	   exptokseq[i] = jj_expentries.get(i);
	 }
	 return new ParseException(token, exptokseq, tokenImage);
  }

  private boolean trace_enabled;

/** Trace enabled. */
  final public boolean trace_enabled() {
	 return trace_enabled;
  }

  /** Enable tracing. */
  final public void enable_tracing() {
  }

  /** Disable tracing. */
  final public void disable_tracing() {
  }

}
